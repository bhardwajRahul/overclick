...
| Tool | What it does |
|---|---|
| `project_list` | the workspace projects, one row each with `id_prefix`, `name` and `repo_url`: what it takes to pick one, since every `project_id` accepts the prefix. `view: "full"` adds the uuid, the organization, `has_context`, the next card number, card counts by status and `created_at`. The markdown stays out of both. Measured on a real board (OCL-208, 61 projects): 24,202 characters in full, 6,214 by default |
...
| `task_create` | creates the card (`mission` is an existing mission id, `mode` solo\|team) with no harness: *what / why / how to confirm* are the whole contract, stay required, and the claim records what runs it. `como_confirmo` takes the list of `{step, expected}` or text with one `step → expected` per line, stored alike; a line missing either side is refused by number (OCL-213). `origem` is optional: omitted, the card records the token that filed it. A `harness` (or `subtasks[].harness`) sent by an older client is ignored and answered with a `warnings` entry for one release, never refused; by default it returns only the short id, status, `updated_at` and generated `changed` fields. Pass `return: "full"` for the card and subtasks. `supersedes` atomically discards an in-execution predecessor, and `inherit: true` reuses its contract without copying comments |
| | `project_id` takes the project uuid **or** its card prefix (`AGB`) and always wins, also when it names a project of another repository than the one the agent works in |
| | Without `project_id`, `repo` names the repository the card is about and the board finds the project itself (OCL-208), so filing a card needs no `project_list` first. `repo` takes the git remote in any form git prints it (`https`, `ssh`, `git@host:owner/repo`, `owner/repo`; the host is ignored, so an ssh alias matches too), or the path the agent works in: a path inside a checkout registered as `file://` matches it, the innermost one first, and otherwise a folder of the path named like a repository matches it, the deepest folder first (`app/.worktrees/x` finds `app`). No match is `NOT_FOUND` saying what was looked for and offering `project_create`; several projects at the same step is `INVALID_ARGUMENT` listing only those candidates. Neither `project_id` nor `repo` is `INVALID_ARGUMENT`. The answer carries `project: {id_prefix, from}`, where `from` is `project_id` or `repo`; with `repo` it adds `match` (`remote`, `path` or `name`) and the `repo_url` that matched, so a card filed in the wrong project shows in the answer |
...
| `task_update` | progress, comment (`comment_kind` supports `report` to increment `reports_count`), `resolved_in` metadata (string or `null`), the `revisado` mark, or a `usage` block that fills or corrects the latest attempt's telemetry, even after deliver/discard. The default response is a compact acknowledgement with the changed fields; pass `return: "full"` for the complete card and usage details. On a `feito` card, `status: "validado"` requires a nonblank `comment` citing who validated in chat and what they said; the timeline labels it as human validation recorded by the agent and shows the agent as author. The status and citation are stored together. Other source states are refused; only the board UI can desvalidate. Managed tokens may send `status: descartado` to discard a card whose reason to exist went away, with the reason in `comment` (required unless `superseded_by` names the card that continues it); the reason lands on the timeline as `Discarded: …` and, for a card in execution, as the abandoned attempt's note. One rule for the board UI and MCP (OCL-203): an open or delivered card can be discarded, and so can one in execution under the caller's own claim, an expired claim, or with a continuation. A live claim held by another executor is refused with `INVALID_TRANSITION`, naming who holds it and when it expires, and pointing to `task_release` first; a validated or already discarded card is refused with why. A `harness` sent by an older client is ignored with a `warnings` entry for one release; the rest of the update still applies |
...
reading the database. Once a project has its `repo_url`, `task_create` with `repo` finds
it without listing anything. Reorganizing later is the same surface: `project_update` renames,
