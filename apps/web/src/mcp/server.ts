...
const DESCRIPTIONS: Record<McpToolName, string> = {
  ...
  task_reopen:
    "Rejects a delivered card (feito) with a required reason, records it as a report and returns the card to aberto for a new task_claim. Preserves previous attempts and deliveries. The next executor receives the reason in its briefing. Cannot validate cards; validation remains human-only in the board UI.",
  task_deliver:
