...
export const SERVER_INSTRUCTIONS = [
  ...
  "",
  "The board records which harness ran a card and never decides one: a card is born without a harness, task_claim records the executor that actually runs it (cli, model, effort) and task_deliver adds the measured usage per model. The per-task Harness table people edit lives in the Overclock app (overclock_list kind: harness), not here.",
  "",
  "Typical flow: task_list shows the queue, task_claim takes a card (send your executor: cli, exact session model from --model, effort, session_id) and returns the briefing, then create and push a commit before task_deliver. A modified working tree is not a delivery; send the commit hash and branch with summary, evidence and usage. When the CLI recipe yields tokens_per_model, measure from the transcript; estimated: true is refused unless you send the reason the recipe prints when the transcript is missing. CLIs whose recipe yields no_tokens may estimate.",
  ...
  "Cards live in projects. task_create finds the project from repo (your git remote, owner/repo or the path you work in), so no project_list is needed first; project_id (uuid or card prefix, for example AGB) wins when sent, also for another repository's project. project_list shows each project's prefix, name and repo_url (view: full for the rest) and project_create starts one.",
...
const DESCRIPTIONS: Record<McpToolName, string> = {
  ...
  project_list:
    "Lists the workspace projects: id_prefix, name and repo_url each. view: full adds id, organization, has_context, next_number, card counts by status and created_at. Filter by organization to read one business at a time. Filing a card does not need it: task_create takes repo and finds the project.",
  project_get:
  ...
  task_list:
    "The card queue of the workspace. Each row defaults to the operational minimum — short_id, title, type, status, priority, cost_usd, plus delivery_unverified only when true — with no uuid on the line, since every tool already accepts short_id. Add include for the rest, in groups: [\"ids\"] (the card's own id), [\"refs\"] (mission_id, project_id, branch, claimed_by), [\"delivery\"] (commit, delivery_verification, delivery_warning, reports_count, revisado, devolve_para), or [\"all\"] for every group at once. Filters: project, mission, organization, resolved_in (exact release), status, priority, created_after (exclusive ISO timestamp), awaiting_review_by. order: newest returns recent cards first; oldest is the default. limit defaults to 50 (max 200); use next_offset as offset for the next page when truncated is true. claimed_by: me requires the executor session_id from task_claim, isolating panes that share a token. Use claimed_by: token only to explicitly list all active claims owned by that token.",
  task_get:
    "Returns the task contract by default, with executor (cli, model, effort): what the card's latest task_claim ran on. Use view: briefing or include: [briefing, usage_recipe, mission] for the full prompt. include: [delivery] returns the latest delivery with full summary, evidence and artifact content. Read previous deliveries with delivery_offset and delivery_limit (default 1, maximum 20), newest first; next_delivery_offset continues the history. view: full includes the latest delivery too.",
  task_search:
  ...
  task_create:
    "Creates a card. o_que, por_que and como_confirmo are the contract and stay required: write them for this card and leave out what the board already has — the mission's context reaches the executor through the briefing, so reference the mission instead of pasting it. como_confirmo takes a list of {step, expected} or text with one \"step → expected\" per line; both are stored alike. The board fills what it knows: the project from repo when project_id is not sent (the answer's project says which), and origem from the token when omitted (send origem only for what it cannot know, such as reportado_por). No harness: the Overclock app decides what runs the card and task_claim records what did. Returns a compact acknowledgement by default; pass return: full for the complete card and subtasks. mission is an existing mission id (mission_create, mission_list); omit it and the card stands on its own. mode is solo or team. supersedes atomically discards the card that was in execution, and inherit reuses its contract without carrying its comments over.",
  task_claim:
    "Takes the card (status becomes em_execucao), opens an ExecutionAttempt and returns the briefing. Declare the exact cli, model and effort this session actually runs with: the board records them on the card as its executor, the harness that ran it. Codex must declare the exact model it was given in --model (gpt-5.6-sol, for example), never just gpt-5. A claim left without activity past the workspace timeout is abandoned as stale and can be taken over without force; the response marks reclaimed_stale: true.",
  task_release:
  ...
  task_update:
    "Records progress, leaves a comment, marks revisado, or reports and corrects the card usage, including on a discarded card. By default it returns a compact acknowledgement with changed fields; pass return: full for the complete card and usage details. project_id moves the card between projects. status: validado on a feito card requires comment quoting who validated and what they said; the agent and citation appear in the timeline. status: descartado discards a card with the reason in comment (required unless superseded_by names the card that continues it): an open or delivered card, or one in execution under your own claim, an expired one, or a continuation. A live claim held by another executor is refused with who holds it and what to do first (task_release). Needs can_manage.",
  task_reopen:
    "Rejects a delivered card (feito) with a required reason, records it as a report and returns the card to aberto for a new task_claim. Preserves previous attempts and deliveries. The next executor receives the reason in its briefing. Validation given by a human in chat can be recorded by an agent via task_update with status: validado and a citation.",
  task_deliver:
    "Delivers the result: summary, evidence, artifacts, commit, branch and usage. By default it returns a compact acknowledgement with the handoff id and delivery telemetry; pass return: full for the task and handoff. Create and push the commit before calling this tool; a modified working tree is not a delivery. The board verifies the commit against the project remote when possible and accepts failures with a delivery_unverified warning. usage is MANDATORY: when the CLI recipe yields tokens_per_model, send the measured segments; estimated: true is refused unless usage.reason is the message the recipe prints when it cannot read the transcript. CLIs whose recipe yields no_tokens may estimate. Instead of retyping the numbers you may omit usage and send transcript.path: the OverClick plugin fills usage on your machine, and the board measures it itself when it can read that path; usage you send always wins. Duration is measured by the server, from the claim to the deliver. how_to_verify (a URL or a command) opens the plain-language validation panel. Status becomes feito.",
  task_delete:
